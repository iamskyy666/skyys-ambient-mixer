export class UI {
  ...
  createCustomPresetBtn(name, presetId) {
    ...
    ${name}
    <span
      class="delete-preset absolute -top-2 -right-2 w-6 h-6 bg-red-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center"
  ...
  updateVolumeDisplay(soundId, volume) {
    ...
    if (card) {
      // Update numb. display
      const volValue = card.querySelector(".volume-value");
      if (volValue) {
        volValue.textContent = volume;
      }

      // Update volume-bar UI
      const volBarFill = card.querySelector(".volume-bar-fill");
      if (volBarFill) {
        volBarFill.style.width = `${volume}%`;
      }
  ...
  // Reset/Stop all UI elements to default state
  resetUi() {
    // Reset sliders to 0
    ...
    // Reset all play-btns to play state
    ...
    // Reset the play/pause btn
    this.updateMainPlayButton(false);

    // Reset master-vol. to 100%
  ...
  setActivePreset(presetKey) {
    // remove active class from akk btns
    document
